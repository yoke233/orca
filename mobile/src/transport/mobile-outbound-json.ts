export const MOBILE_OUTBOUND_MAX_JSON_BYTES = 4 * 1024 * 1024

export function stringifyMobileOutboundJson(
  value: unknown,
  maxBytes = MOBILE_OUTBOUND_MAX_JSON_BYTES
): string {
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 1) {
    throw new Error('Mobile outbound JSON limit must be a positive safe integer')
  }
  ...
  const serialized = JSON.stringify(value, function (key, item: unknown) {
    ...
    estimatedBytes += inArray && isOmittedObjectValue(item) ? 4 : jsonValueBytes(item)
    if (estimatedBytes > maxBytes) {
      throw new Error(`Mobile outbound JSON exceeds ${maxBytes} bytes`)
    }
    return item
  })
  if (serialized === undefined) {
    throw new Error('Mobile outbound value is not JSON serializable')
  }
  return serialized
}
...
function jsonValueBytes(value: unknown): number {
  ...
  if (typeof value === 'object' && value !== null) {
    return boxedPrimitiveJsonBytes(value) ?? 2
  }
  return 4
}
