...
import type { QuickOpenFilesystem } from './quick-open-directory-reader'
...
async function hasGitEntry(
  absPath: string,
  filesystem: Pick<QuickOpenFilesystem, 'lstat'>
): Promise<boolean> {
  try {
...
export async function classifyQuickOpenGitEntry(
  rootPath: string,
  entry: string,
  filesystem: Pick<QuickOpenFilesystem, 'lstat'> = nodeFsPromises
