...
import { UsageBreakdownSection, type UsageBreakdownRow } from './UsageBreakdownSection'
import { UsageRecentSessionsTable } from './UsageRecentSessionsTable'
import { translate } from '@/i18n/i18n'

type MuseUsageDetailsProps = {
  ...
  summary: MuseUsageSummary | null | undefined
}

function toBreakdownRows(rows: MuseUsageBreakdownRow[]): UsageBreakdownRow[] {
  return rows.map((row) => ({
    key: row.key,
    label: row.label,
...
export function MuseUsageDetails({
  daily,
  modelBreakdown,
  projectBreakdown,
  recentSessions,
  summary
}: MuseUsageDetailsProps): React.JSX.Element {
  return (
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <UsageBreakdownSection
          ...
          rows={toBreakdownRows(modelBreakdown)}
        ...
        <UsageBreakdownSection
          ...
          rows={toBreakdownRows(projectBreakdown)}
          eventsOrTurns="events"
        />
      </div>
      ...
      <UsageRecentSessionsTable
        ...
        description={translate(
          'auto.components.stats.MuseUsagePane.recentSessionsDescription',
          'Most recent local Muse sessions in this scope.'
        )}
        headings={[
