...
function isCodexRolloutInsideSessionsRoot(sessionsRoot: string, filePath: string): boolean {
  ...
  const relativePath = relativePathInsideRoot(
    normalizeRuntimePathSeparators(comparisonSessionsRoot),
    normalizeRuntimePathSeparators(comparisonFilePath)
  )
...
function resolveTrustedCodexSessionResume(args: {
  ...
  for (const homePath of args.trustedCodexHomes) {
    const sessionsRoot = (homePath.includes('\\') ? win32 : posix).join(homePath, 'sessions')
