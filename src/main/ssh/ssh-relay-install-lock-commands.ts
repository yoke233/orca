...
function posixStealInstallLockCommand(lockDir: string, staleAfterSeconds: number): string {
  ...
  return [
    ...
    'steal_parent=${steal_root%/*};',
    'steal_leaf=${steal_root##*/};',
    '[ "$steal_parent" != "$steal_root" ] || steal_parent=.;',
    `trap 'find "$steal_parent" -mindepth 1 -maxdepth 1 -name "$steal_leaf.*" -exec rm -rf {} + 2>/dev/null || true; rm -rf "$lock_tombstone" 2>/dev/null || true' EXIT;`,
